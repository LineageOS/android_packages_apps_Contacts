...
public class MemoryStatusActivity extends ContactsActivity {
    ...
    private class AccountListAdapter extends BaseAdapter {
        ...
        private void bindView(int position, View convertView, ViewGroup parent,
            ...
            CharSequence label = accountType.getDisplayLabel(accountContext);
            // Handle the local accounts as a special case. They don't exist in AccountTypeManager.
            if (label == null && PhoneAccountType.ACCOUNT_TYPE.equals(filter.accountType)) {
                label = accountContext.getResources().getString(R.string.local_storage_account);
            }

            viewCache.accountName.setText(label
                    + "<" + filter.accountName + ">");
